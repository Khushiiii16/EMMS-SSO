 ...
 */
	...
	$.extend($.fn.validatebox.defaults.rules, {  
	   mobile: {
	      ...
	      message: 'Enter Mobile No [e.g (9911223344) ].'
	   }  
	});
	
	/**
	 * telephone	: Validates email id 
	 * USAGE		: email
	 */
	$.extend($.fn.validatebox.defaults.rules, {  
	   email: {
	...
	$.extend($.fn.validatebox.defaults.rules, {  
		passwordStrength: {  
			validator: function(value,param){				
	        	...
	        	r=/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?!.*012)(?!.*123)(?!.*234)(?!.*345)(?!.*456)(?!.*567)(?!.*678)(?!.*789)(?!.*([0-9])\\1{2})(?=.*[#@!%&_])[A-Za-z\d#@!%&_]{8,20}$/;
	        	 
	        	var isSpecialPresent = value.match(r);	
	        	
				if(param[0]==3 && (!isSpecialPresent))
				{
					param[1] = "Password must contain at least one Uppercase Alphabet,one Lowercase Alphabet, one Number and one Special Character.";
					return false;
				}
				return true;
			},  
